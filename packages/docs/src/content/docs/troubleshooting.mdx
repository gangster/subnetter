...
description: "This guide provides solutions for common issues encountered when using the Subnetter tool. It covers configuration problems, allocation failures, and integration challenges."
---

This guide provides solutions for common issues encountered when using the Subnetter tool. It covers configuration problems, allocation failures, and integration challenges.
...
subnetter generate config.json --log-level debug

# Include timestamps in log output
subnetter generate config.json --log-level debug --timestamps

# Combine with other options
subnetter generate config.json --output custom.csv --log-level trace --timestamps
```
Available log levels (from least to most verbose):
- `silent`: Disable all logging
- `error`: Show only errors
- `warn`: Show warnings and errors
- `info`: Show standard informational messages (default)
- `debug`: Show detailed debugging information
- `trace`: Show very detailed trace information for all operations

Each component of Subnetter (Config, Allocator, etc.) logs its operations at the appropriate level, allowing you to trace the execution flow when troubleshooting.

### Programmatic Debugging

If you're using Subnetter programmatically, you can add more detailed error handling and configure logging:
...
const logger = createLogger('MyApplication');
...
try {
  ...
  } else if (error.name === 'AllocationError') {
    logger.error('CIDR allocation failed:', error.message);
  } else if (error.name === 'CidrError') {
    logger.error('CIDR calculation error:', error.message);
  } else if (error.name === 'OutputError') {
    logger.error('Output generation failed:', error.message);
  } else {
...
If you encounter a persistent issue, create a minimal reproduction case:

1. Start with a simple configuration that works
2. Gradually add elements from your problematic configuration
3. Identify the specific change that causes the issue
4. Report the issue with your minimal reproduction case

## Getting Additional Help

If you're still experiencing issues after trying the solutions in this guide:

1. Check the [GitHub repository](https://github.com/gangster/subnetter) for known issues
