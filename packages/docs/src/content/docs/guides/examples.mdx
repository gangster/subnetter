...
<Aside type="tip" title="Subnet Type Order">
Notice that `Application` (the largest at `/22`) comes first. Always order subnet types from largest to smallest to prevent CIDR overlaps.
</Aside>
