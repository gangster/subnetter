...
| Configuration | AZ CIDR Size | Node Subnet | Load Balancer | Endpoints | Transit | Reserved | Node Capacity per AZ | Efficiency | Notes |
|---------------|-------------|-------------|---------------|-----------|---------|----------|---------------------|------------|-------|
| Small | /26 | /27 | /29 | /29 | /30 | /30 | 30 | High | Good for small clusters, minimal wasted space |
| Medium | /25 | /26 | /28 | /28 | /29 | /28 + /29 | 62 | High | Balanced size for most deployments |
| Large | /24 | /25 | /27 | /27 | /28 | /26 | 126 | Medium | Good for larger deployments |
| **Recommended** | **/25** | **/26** | **/28** | **/28** | **/29** | **/28 + /29** | **62** | **High** | **Optimal for most deployments** |

### Recommended Configuration Details

Our recommended configuration uses a /25 CIDR block (128 addresses) for each AZ with the following allocation:

- **Node Subnet: /26** — For Kubernetes nodes (64 addresses, 62 usable IPs)
- **Load Balancer Subnet: /28** — For load balancers (16 addresses, 14 usable IPs)
- **Endpoints Subnet: /28** — For VPC endpoints (16 addresses, 14 usable IPs)
- **Transit Subnet: /29** — For transit gateway attachments (8 addresses, 6 usable IPs)
- **Reserved Space: /28 + /29** — For future expansion (24 addresses total: 16 + 8)

**Total: 64 + 16 + 16 + 8 + 24 = 128 addresses (exactly one /25)**
...
| Subnet Type | CIDR Block | Addresses | Usable IPs | Purpose |
|-------------|------------|-----------|------------|---------|
| Node | 10.0.0.0/26 | 64 | 62 | Kubernetes nodes |
| Load Balancer | 10.0.0.64/28 | 16 | 14 | Kubernetes service exposure |
| Endpoints | 10.0.0.80/28 | 16 | 14 | VPC endpoint IPs |
| Transit | 10.0.0.96/29 | 8 | 6 | Transit gateway attachment |
| Reserved 1 | 10.0.0.104/29 | 8 | 6 | Future expansion |
| Reserved 2 | 10.0.0.112/28 | 16 | 14 | Future expansion |

### Subnet Allocations for AZ2 (us-east-1b)

| Subnet Type | CIDR Block | Addresses | Usable IPs | Purpose |
|-------------|------------|-----------|------------|---------|
| Node | 10.0.0.128/26 | 64 | 62 | Kubernetes nodes |
| Load Balancer | 10.0.0.192/28 | 16 | 14 | Kubernetes service exposure |
| Endpoints | 10.0.0.208/28 | 16 | 14 | VPC endpoint IPs |
| Transit | 10.0.0.224/29 | 8 | 6 | Transit gateway attachment |
| Reserved 1 | 10.0.0.232/29 | 8 | 6 | Future expansion |
| Reserved 2 | 10.0.0.240/28 | 16 | 14 | Future expansion |

### Subnet Allocations for AZ3 (us-east-1c)

| Subnet Type | CIDR Block | Addresses | Usable IPs | Purpose |
|-------------|------------|-----------|------------|---------|
| Node | 10.0.1.0/26 | 64 | 62 | Kubernetes nodes |
| Load Balancer | 10.0.1.64/28 | 16 | 14 | Kubernetes service exposure |
| Endpoints | 10.0.1.80/28 | 16 | 14 | VPC endpoint IPs |
| Transit | 10.0.1.96/29 | 8 | 6 | Transit gateway attachment |
| Reserved 1 | 10.0.1.104/29 | 8 | 6 | Future expansion |
| Reserved 2 | 10.0.1.112/28 | 16 | 14 | Future expansion |

## IP Space Efficiency Analysis

The table below summarizes the IP space efficiency of the recommended configuration:

| Resource Level | Total IPs | Active Subnets | Reserved | Utilization % |
|----------------|-----------|----------------|----------|---------------|
| Region (us-east-1) | 512 | 312 (3 × 104) | 200 | 61% |
| Per AZ | 128 | 104 | 24 | 81% |
| All 3 AZs | 384 | 312 | 72 | 81% |
| Region Reserved | 128 | 0 | 128 | 0% |

**Note**: Active subnets include Node (/26=64), Load Balancer (/28=16), Endpoints (/28=16), and Transit (/29=8) = 104 addresses per AZ. The remaining 24 addresses per AZ are reserved for future expansion.
...
2. Run Subnetter to generate the allocation table:
   ```bash
   subnetter generate --config kubernetes-config.yaml --output kubernetes-allocations.csv
   ```
