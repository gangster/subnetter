...
description: Integrate Subnetter with NetBox for bidirectional IP address management synchronization.
---
...
<Aside type="note">
In NetBox 4.x, the prefix-to-site relationship uses `scope_type` and `scope_id` instead of the deprecated `site` field.
</Aside>
...
### Phase 1: Export (MVP) <Badge text="In Progress" variant="note" />
