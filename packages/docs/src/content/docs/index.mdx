---
title: Subnetter
description: Automated CIDR allocation for cloud infrastructure
template: splash
hero:
  tagline: Plan your entire cloud network in minutes, not days. Generate thousands of conflict-free subnet allocations from a single config file.
  actions:
    - text: Get Started
      link: /subnetter/user-guide/
      icon: right-arrow
      variant: primary
    - text: View on GitHub
      link: https://github.com/gangster/subnetter
      icon: external
---

import { Card, CardGrid, Tabs, TabItem, Aside, Steps, Badge } from '@astrojs/starlight/components';
import MermaidDiagram from '../../components/MermaidDiagram.astro';

## The Problem

Every cloud network starts with a simple question: *"What IP addresses should we use?"*

What seems straightforward quickly becomes a nightmare at scale. A typical enterprise cloud deployment requires:

<CardGrid>
  <Card title="Exponential Complexity" icon="warning">
**12 accounts** × **4 regions** × **3 AZs** × **6 subnet types** = **864 unique subnets**

Each one needs a carefully calculated CIDR block. One overlap, and traffic routes to the wrong place—or nowhere at all.
  </Card>
  <Card title="The Spreadsheet Trap" icon="document">
Teams resort to massive spreadsheets. Formulas break. Rows get deleted. Version control is "final_v3_REAL.xlsx". The person who built it left the company.
  </Card>
</CardGrid>

<CardGrid>
  <Card title="Growth Breaks Everything" icon="error">
You allocated a /24 for production. Now you need 500 hosts. Re-IP the entire VPC? Migrate workloads? Explain to leadership why this "simple change" takes 3 months?
  </Card>
  <Card title="Multi-Cloud Multiplies Pain" icon="puzzle">
AWS uses `us-east-1a`. Azure uses `eastus-1`. GCP uses `us-central1-a`. Three clouds means three naming conventions, three spreadsheets, and three times the chance for human error.
  </Card>
</CardGrid>

### The Hidden Costs

- **Debugging connectivity issues** caused by overlapping CIDRs: hours to days
- **Re-IP projects** when you outgrow your allocation: weeks to months  
- **Onboarding new engineers** to understand the network: "ask Sarah, she knows"
- **Audit failures** because no one can explain why this subnet exists
- **Merger/acquisition integration** with conflicting IP schemes: organizational trauma

### There's a Better Way

What if you could describe your entire network structure in a simple config file—and generate every subnet allocation automatically, with mathematical guarantees of zero conflicts?

**That's Subnetter.**

Define your accounts, regions, and subnet types once. Subnetter handles the CIDR math, respects cloud provider conventions, and produces a complete allocation table in seconds. Add a new region? One line in the config. Need to audit your IP space? It's all in version control.

---

## Try It Now

Get your first allocation in under a minute:

<Tabs>
  <TabItem label="npm">
```bash
# Install and run
npm install -g subnetter
subnetter generate --config config.json --output allocations.csv
```
  </TabItem>
  <TabItem label="npx (no install)">
```bash
# Run directly without installing
npx subnetter generate --config config.json --output allocations.csv
```
  </TabItem>
</Tabs>

<Aside type="tip" title="Quick Example">
  Download a [sample config](https://github.com/gangster/subnetter/tree/main/examples) and try it yourself!
</Aside>

---

## See It In Action

<CardGrid>
  <Card title="📄 Your Config (15 lines)" icon="document">
```json
{
  "baseCidr": "10.0.0.0/8",
  "accounts": [
    { "name": "production", "clouds": { "aws": { "regions": ["us-east-1", "us-west-2", "eu-west-1"] } } },
    { "name": "staging", "clouds": { "aws": { "regions": ["us-east-1"] } } },
    { "name": "development", "clouds": { "azure": { "regions": ["eastus", "westeurope"] } } }
  ],
  "subnetTypes": {
    "Public": 24,
    "Private": 24,
    "Data": 26,
    "Management": 28
  }
}
```
  </Card>
  <Card title="📊 Your Output (144 rows)" icon="list-format">
| Account | Cloud | Region | AZ | Subnet | CIDR |
|---------|-------|--------|-----|--------|------|
| production | aws | us-east-1 | us-east-1a | Public | 10.0.0.0/24 |
| production | aws | us-east-1 | us-east-1a | Private | 10.0.1.0/24 |
| production | aws | us-east-1 | us-east-1a | Data | 10.0.2.0/26 |
| production | aws | us-east-1 | us-east-1a | Management | 10.0.2.64/28 |
| production | aws | us-east-1 | us-east-1b | Public | 10.0.4.0/24 |
| ... | ... | ... | ... | ... | ... |
| development | azure | westeurope | westeurope-3 | Management | 10.2.28.64/28 |

**3 accounts × 6 regions × 3 AZs × 4 subnets = 144 allocations**
  </Card>
</CardGrid>

<Aside type="note" title="Zero Conflicts Guaranteed">
Every CIDR is mathematically guaranteed to be non-overlapping. The hierarchical allocation algorithm ensures each account, region, AZ, and subnet gets its own dedicated slice of IP space.
</Aside>

---

## Real-World Scenarios

<Tabs>
  <TabItem label="🏢 Enterprise">
**Scenario:** Global financial services company migrating to cloud

- 50+ AWS accounts (prod, staging, dev, sandbox per business unit)
- 8 regions for data residency compliance
- 6 subnet types per VPC (public, private, data, management, endpoints, transit)
- Strict audit requirements for IP allocation

**Without Subnetter:** 6-month planning project. Dedicated network team. Custom tooling.

**With Subnetter:** Single config file. Version-controlled. Auditable. Done in a day.
  </TabItem>
  <TabItem label="🚀 Startup">
**Scenario:** Fast-growing SaaS company expanding internationally

- Started with 1 region, now need 5
- Adding new microservices weekly
- Can't afford downtime for re-IP projects
- Small team wearing many hats

**Without Subnetter:** Outgrow your IP scheme. Painful re-architecture. Production incidents.

**With Subnetter:** Plan for 10x growth upfront. Add regions in minutes. Sleep at night.
  </TabItem>
  <TabItem label="☁️ Multi-Cloud">
**Scenario:** Healthcare company with multi-cloud strategy

- AWS for compute, Azure for AI/ML, GCP for analytics
- Need consistent IP strategy across all three
- Each cloud has different AZ naming conventions
- Compliance requires documented network architecture

**Without Subnetter:** Three different spreadsheets. Manual translation. Inevitable conflicts.

**With Subnetter:** One config. All clouds. Provider-aware AZ naming. Single source of truth.
  </TabItem>
</Tabs>

---

## How It Works

Subnetter divides your IP space hierarchically—no overlaps, guaranteed:

<MermaidDiagram chart={`
flowchart LR
    A["🌐 Base CIDR<br/>10.0.0.0/8<br/><i>16M addresses</i>"] --> B["☁️ Accounts<br/>/16 each<br/><i>65K per account</i>"]
    B --> C["🗺️ Regions<br/>/20 each<br/><i>4K per region</i>"]
    C --> D["📍 AZs<br/>/22 each<br/><i>1K per AZ</i>"]
    D --> E["🔲 Subnets<br/>/24-/28<br/><i>16-254 per subnet</i>"]
    
    style A fill:#1a1a2e,color:#fff,stroke:#4a4a6a,stroke-width:2px
    style B fill:#16213e,color:#fff,stroke:#4a4a6a,stroke-width:2px
    style C fill:#0f3460,color:#fff,stroke:#4a4a6a,stroke-width:2px
    style D fill:#1a5276,color:#fff,stroke:#4a4a6a,stroke-width:2px
    style E fill:#1e8449,color:#fff,stroke:#4a4a6a,stroke-width:2px
`} />

<CardGrid>
  <Card title="Deterministic" icon="approve-check-circle">
    Same config always produces the same output. Perfect for GitOps, disaster recovery, and reproducible infrastructure.
  </Card>
  <Card title="Extensible" icon="add-document">
    Add new accounts or regions anytime. Subnetter finds the next available space without touching existing allocations.
  </Card>
  <Card title="Provider-Aware" icon="puzzle">
    Knows that AWS uses `us-east-1a`, Azure uses `eastus-1`, and GCP uses `us-central1-a`. You don't have to.
  </Card>
  <Card title="Validation Built-In" icon="approve-check">
    Catches errors before they reach production: overlapping CIDRs, invalid prefixes, exhausted address space.
  </Card>
</CardGrid>

---

## Why Teams Choose Subnetter

<CardGrid>
  <Card title="🕐 Save Days of Planning" icon="rocket">
    What takes a team days in spreadsheets takes Subnetter seconds. Focus your engineers on building, not IP math.
  </Card>
  <Card title="🛡️ Eliminate Human Error" icon="approve-check-circle">
    No more typos in CIDR notation. No more "oops, that overlaps with prod." Mathematically guaranteed correctness.
  </Card>
  <Card title="📈 Scale Confidently" icon="up-caret">
    Plan for 10x growth from day one. Add accounts, regions, and subnets without re-architecting your network.
  </Card>
  <Card title="📋 Audit-Ready Documentation" icon="document">
    Your network allocation is a version-controlled config file. Every change is tracked. Every allocation is explainable.
  </Card>
</CardGrid>

---

## The Manual Approach vs. Subnetter

| Aspect | Manual (Spreadsheets) | Subnetter |
|--------|----------------------|-----------|
| **Time to plan 100 subnets** | Hours to days | Seconds |
| **Risk of overlap** | High (human error) | Zero (algorithmic) |
| **Adding a new region** | Recalculate everything | Add one line to config |
| **Multi-cloud consistency** | Three separate docs | One unified config |
| **Onboarding new team members** | "Ask Sarah, she knows" | Self-documenting config |
| **Disaster recovery** | Hope the spreadsheet is current | Regenerate from Git |
| **Audit trail** | "Check the wiki" | Git history |

---

## Choose Your Path

<CardGrid>
  <Card title="🚀 New to Subnetter?" icon="rocket">
    Start with the **User Guide** for installation, configuration basics, and your first allocation.
    
    [Get Started →](/subnetter/user-guide/)
  </Card>
  
  <Card title="📋 Ready to Configure?" icon="setting">
    Jump to the **Configuration Reference** for the complete schema and all available options.
    
    [View Config Reference →](/subnetter/configuration/)
  </Card>
  
  <Card title="🔧 Using Programmatically?" icon="seti:typescript">
    Check the **API Documentation** to integrate Subnetter into your Node.js applications.
    
    [Browse API Docs →](/subnetter/api/overview/)
  </Card>
  
  <Card title="🎓 Learn CIDR Basics?" icon="open-book">
    New to IP subnetting? The **CIDR Primer** covers everything you need to know.
    
    [Read CIDR Primer →](/subnetter/cidr-primer/)
  </Card>
</CardGrid>

---

## Key Capabilities

<CardGrid>
  <Card title="Multi-Cloud Native" icon="cloud-download">
    First-class support for **AWS**, **Azure**, and **GCP** with provider-specific region and AZ naming conventions built in.
  </Card>
  
  <Card title="Flexible Subnet Sizing" icon="list-format">
    Define different prefix lengths for different workloads. Large /24s for compute, small /28s for management.
  </Card>
  
  <Card title="Account-Level Overrides" icon="pencil">
    Need a specific CIDR for one account? Override defaults without breaking the hierarchy.
  </Card>
  
  <Card title="CSV & Programmatic Output" icon="seti:json">
    Export to CSV for spreadsheet users, or use the TypeScript API for automation pipelines.
  </Card>
</CardGrid>

---

## More Resources

<CardGrid>
  <Card title="Architecture" icon="puzzle">
    Understand Subnetter's technical design and allocation algorithms.
    
    [View Architecture →](/subnetter/architecture/)
  </Card>
  
  <Card title="Troubleshooting" icon="warning">
    Solutions for common issues and error messages.
    
    [Troubleshooting Guide →](/subnetter/troubleshooting/)
  </Card>
  
  <Card title="Kubernetes Guide" icon="seti:docker">
    Specialized network design for multi-AZ Kubernetes deployments.
    
    [K8s Network Design →](/subnetter/guides/kubernetes-network-design/)
  </Card>
  
  <Card title="Developer Guide" icon="github">
    Contributing to Subnetter? Start here.
    
    [Developer Guide →](/subnetter/developer-guide/)
  </Card>
</CardGrid>
