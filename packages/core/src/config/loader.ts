...
export class ConfigValidationError extends Error {
  constructor(message: string, public errors: ZodError) {
    ...
    logger.debug(`Config validation error: ${message}`);
    logger.debug('Validation issues:');
    errors.errors.forEach((error, index) => {
      logger.debug(`  ${index + 1}. Path: ${error.path.join('.')} - ${error.message}`);
    });
  }
}
 ...
 * @param extension File extension (e.g., 'on', '.yaml')
...
function parseConfigData(data: string, extension: string): unknown {
  ...
  switch (extension.toLowerCase()) {
    ...
    default:
      ...
      throw new Error(`Unsupported file extension: ${extension}. Only JSON and YAML formats are supported.`);
  }
}
 ...
 * @throws {Error} If the file can't be read or has an unsupported format
 */
export function loadConfig(configPath: string): Config {
  ...
  try {
    ...
    logger.error(`Error loading configuration: ${error instanceof Error ? error.message : 'Unknown error'}`);
    throw error;
  }
}
