...
/**
 * Error thrown during CSV output generation.
 */
 ...
 * @throws {OutputError} If the CSV file cannot be written
 */
export async function writeAllocationsToCsv(allocations: Allocation[], outputPath: string): Promise<void> {
  ...
  try {
    ...
    throw new OutputError(`Failed to write allocations to CSV: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
